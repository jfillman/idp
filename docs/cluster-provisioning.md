...
## Reference: secrets a human must plant per cluster

None of these are generated by `hack/customize-cluster.sh` or synced from any
`ExternalSecret` — a real, live audit of kind-dev and kind-prod (2026-09-01) found
these are the only secrets on either cluster with no `ExternalSecret` backing them
that are actually load-bearing for bootstrap (everything else either self-heals —
operator-generated webhook/TLS certs, ArgoCD's own auto-generated `argocd-secret`/
`argocd-redis` — or is a Helm-chart-generated random password that regenerates on a
fresh install).

**Every cluster, regardless of `type`:**

| Secret | Purpose |
|---|---|
| ArgoCD repo-access credential | Needed before `root-app-of-apps.yaml` can sync anything at all. Either the shared platform GitHub App's credentials (if reusing it — see [platform-cicd's secrets-management.md](../../platform-cicd/docs/admin/secrets-management.md#github-credentials-in-this-platform)) or a standalone classic PAT as `argocd-repo-creds-<you>` (the template's own bootstrap sequence calls this out by name) |
| `registry-credentials-dockerconfigjson` (Infisical key) | GHCR pull credential — every app pod's `imagePullSecrets` reference this by name unconditionally; no per-app opt-in |

**Dev-role clusters only** (hosts Infisical + platform-cicd's control plane + the
Bootstrap-tier XRDs):

| Secret | Purpose |
|---|---|
| `infisical-secrets` (`ENCRYPTION_KEY`/`AUTH_SECRET`/`SITE_URL`) | Infisical's own root encryption key. Structurally can never be `ExternalSecret`-managed — Infisical *is* the ESO backend. Losing this without a backup is unrecoverable data loss, not just a re-provisioning inconvenience |
| `infisical-bootstrap-credentials` | Infisical's first-admin email/password/org, consumed by the Helm chart's one-shot `autoBootstrap` Job |
| Infisical's Postgres/Redis passwords | `openssl rand -hex 20` per this repo's own convention — generated fresh per cluster, never reused, committed in plaintext in the `infisical` Application's own `valuesObject` (a deliberate, documented exception — see that file's own header for why the usual `existingSecret` pattern doesn't work with this chart) |
| `fulcio-secret` / `fulcio-server-config` | Per-cluster signing root, generated by `generate-cluster-values.sh` — by design never something a "sync from a backend" model applies to |
| Shared platform GitHub App credentials (`github-application-id`/`github-private-key`/`github-installation-id`, `github-webhook-secret`) | Planted into this cluster's own Infisical project once; ArgoCD repo-creds, Pipelines-as-Code, and `token-review-interceptor` all read from the same values afterward — see [secrets-management.md](../../platform-cicd/docs/admin/secrets-management.md#github-credentials-in-this-platform) for the full consumer list |
| `provider-github-creds` (classic PAT, `repo`+`delete_repo` scope) | Crossplane's `provider-upjet-github`, for the Bootstrap-tier XRDs (`NodeJSApplication`, `ApplicationEnvironment`) to create/delete GitHub repos — a GitHub App can't do this on a personal account, hence a raw PAT here specifically |
| Per-app PR-generator tokens (`<app>-pr-generator-token`, one classic PAT each) | ArgoCD `ApplicationSet`'s `pullRequest.github.tokenRef` for PR-based ephemeral environments. Deliberately hand-minted per app rather than ESO-synced — a prior review of minting these via ESO's Webhook generator was rejected as not worth a persisted-credential tradeoff at this call frequency |
